...
FIELD_GLOSSARY = {
    ...
    "HOLDING_QUANTITY": "Shares actually held at custody (NOMINAL_BASIS (on custody side) minus LOAN_QUANTITY).",
...
SYSTEM_PROMPT = f"""You are prioritizing dividend reconciliation breaks for an ops queue.
Prioritize by business impact and urgency.
Key signal: absolute NET_AMOUNT_SC_DIFF (cash impact).
...
{{
  "reconciliation_breaks": [
    {{"COAC_EVENT_KEY":"...", BANK_ACCOUNT:"...","priority":3,"reason":"short reason"}},
    ...
  ]
}}
Highest-priority first (largest, urgent, custodian-likely).
"""

def prioritize_breaks(
    breaks: List[Dict[str, Any]],
    model: str = None
):
    """Prioritize reconciliation breaks using an LLM."""
    api_key = os.getenv("OPENAI_API_KEY")
    client = OpenAI(api_key=api_key)
    model = model or os.getenv("MODEL") or "gpt-5-mini"  # fallback to a fast model

    #break_json = json.dumps(breaks, ensure_ascii=False, indent=2)

    user = USER_PROMPT_TEMPLATE.format(breaks=breaks)
